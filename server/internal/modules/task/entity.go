...
type TaskResponse struct {
	...
	Tags             []*tag.TagResponse `json:"tags,omitempty"` // <<< ДОБАВЛЕНО поле для тегов
...
func ToTaskResponseList(tasks []*Task) []*TaskResponse {
	if len(tasks) == 0 {
		return []*TaskResponse{} // Возвращаем пустой срез, а не nil, для консистентности JSON
	}
...
const (
	ViewTypeDefault           GetTasksViewType = ""         // Поведение по умолчанию (например, как сейчас - личные созданные)
	ViewTypeUserCentricGlobal GetTasksViewType = "global"   // Для экрана "Все задачи" (назначенные ИЛИ созданные)
	ViewTypeUserPersonal      GetTasksViewType = "personal" // Для экрана "Личные задачи" (team_id IS NULL AND (назначенные ИЛИ созданные))
	// ViewTypeTeamSpecific уже обрабатывается через TeamID в GetTasksParams
)

type GetTasksParams struct {
	UserID           uint
	ViewType         GetTasksViewType // <<< НОВЫЙ ПАРАМЕТР для определения логики выборки
	TeamID           *uint
	Status           *string
	Priority         *int
	AssignedToUserID *uint // Этот параметр будет использоваться в сочетании с ViewType
	// CreatedByUserID  *uint   // Можно добавить, если нужна явная фильтрация по создателю поверх ViewType
...
// GetTasksRequest (в controller) также нужно будет обновить, чтобы принимать view_type
type GetTasksRequest struct {
	ViewType         *GetTasksViewType `form:"view_type" validate:"omitempty,oneof=global personal"` // <<< НОВЫЙ ПАРАМЕТР
	...
	// CreatedByUserID *uint `form:"created_by_user_id"` // Опционально
...
type CreateTaskRequest struct {
	...
	AssignedToUserID *uint      `json:"assigned_to_user_id,omitempty"`
	TeamID           *uint      `json:"team_id,omitempty"`
	UserTagIDs       []uint     `json:"user_tag_ids,omitempty"` // <<< ДОБАВЛЕНО
	TeamTagIDs       []uint     `json:"team_tag_ids,omitempty"` // <<< ДОБАВЛЕНО
}
...
type UpdateTaskRequest struct {
	...
	Deadline         *time.Time `json:"deadline"` // Может быть null для сброса
	...
	AssignedToUserID *uint      `json:"assigned_to_user_id"`    // Может быть null для снятия назначения
	UserTagIDs       *[]uint    `json:"user_tag_ids,omitempty"` // <<< ДОБАВЛЕНО (указатель, чтобы отличить отсутствие от пустого массива)
	TeamTagIDs       *[]uint    `json:"team_tag_ids,omitempty"` // <<< ДОБАВЛЕНО (указатель)
}
...
type PatchTaskRequest struct {
	...
	UserTagIDs       *[]uint    `json:"user_tag_ids,omitempty"` // <<< ДОБАВЛЕНО (указатель)
	TeamTagIDs       *[]uint    `json:"team_tag_ids,omitempty"` // <<< ДОБАВЛЕНО (указатель)
}

// --- Интерфейсы для модуля task ---

type Controller interface {
	CreateTask(w http.ResponseWriter, r *http.Request)
	GetTask(w http.ResponseWriter, r *http.Request)
	GetTasks(w http.ResponseWriter, r *http.Request)
	UpdateTask(w http.ResponseWriter, r *http.Request) // PUT
	PatchTask(w http.ResponseWriter, r *http.Request)  // PATCH
...
type UseCase interface {
	...
	// GetTasks больше не возвращает PaginationResponse
	GetTasks(userID uint, reqParams GetTasksRequest) ([]*TaskResponse, error)
	UpdateTask(taskID uint, userID uint, req UpdateTaskRequest) (*TaskResponse, error) // PUT
	PatchTask(taskID uint, userID uint, req PatchTaskRequest) (*TaskResponse, error)   // PATCH
	DeleteTask(taskID uint, userID uint) error
}

// Repo определяет методы для взаимодействия с хранилищем данных для задач.
type Repo interface {
	CreateTask(taskModel *Task) (*Task, error)
	GetTaskByID(taskID uint, userID uint) (*Task, error) // userID для базовой проверки личных задач
